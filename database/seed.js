...
async function runSeed() {
  ...
  for (const u of users) {
    await insertUser.run([u.username, u.password, u.full_name, u.email, u.phone, u.role, u.avatar_color]);
  }
  ...
  for (const c of clients) {
    await insertClient.run([c.name, c.company, c.email, c.phone, c.industry, c.status, c.source, c.assigned_to, c.estimated_value, c.notes]);
  }
  ...
  for (const b of briefs) {
    await insertBrief.run([b.client_id, b.budget, b.goals, b.target_audience, b.services_interested, b.current_challenges, b.competitors, b.preferences, b.internal_notes]);
  }
  ...
  for (const c of calls) {
    await insertCall.run([c.user_id, c.client_id, c.call_date, c.duration, c.call_type, c.outcome, c.subject, c.notes, c.next_follow_up]);
  }
  ...
  for (const m of meetings) {
    await insertMeeting.run([m.user_id, m.client_id, m.title, m.meeting_date, m.duration, m.location, m.meeting_type, m.status, m.attendees, m.agenda, m.mom, m.decisions, m.action_items, m.next_meeting]);
  }
  ...
  for (const t of todos) {
    await insertTodo.run([t.user_id, t.title, t.description, t.due_date, t.priority, t.status, t.client_id, t.category]);
  }
